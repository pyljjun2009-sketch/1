...
- 输出目录默认 `dist/`；可用环境变量 `DSH_DESKTOP_BUILD_DIR` 指定（建议指向非 OneDrive 同步目录）：

  ```bash
  set DSH_DESKTOP_BUILD_DIR=%LOCALAPPDATA%\dsh-desktop-build&& npm run dist
  ```

- 构建前先 `npm run clean` 并关闭正在运行的 exe；本仓库位于 OneDrive 同步目录，偶发文件锁（`ReplaceFileW EIO`）时重试或改用非同步构建目录。
